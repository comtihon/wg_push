...
-module(wg_transport_logic).

-include("wg_push.hrl").

-define(FORM_DATA(DT, PL, I, ED, PR), <<1, 0, 32, DT/binary, 2, (byte_size(PL)):16/integer, PL/binary,
3, 0, 4, I:32/integer, 4, 0, 4, ED:32/integer, 5, 0, 1, PR:8/integer>>).

-export([pack_items/1, pack_item/1, build_ssl_options/1, open_connection/1]).
-export([encode_aps/1, encode_alert/1]).

-type(item_error() :: {error, integer(), atom()}).

-spec open_connection(map()) -> {ok, port()} | {error, term()}.
open_connection(Config = #{host := Host, port := Port}) ->
  Options = [{active, false}, binary] ++ wg_transport_logic:build_ssl_options(Config),
...
pack_item(#wg_push_item{id = Id, device_token = DeviceToken, payload = PayloadData,
  expiration_date = EDate, priority = Priority}) ->
  Payload = encode_payload(PayloadData),
  if
    byte_size(Payload) > 2048 ->
      {error, Id, payload_too_big};
    byte_size(DeviceToken) /= 32 ->
      {error, Id, invalid_device_token};
    true ->
      Data = ?FORM_DATA(DeviceToken, Payload, Id, EDate, Priority),
      {ok, Data}
  end.

-spec build_ssl_options(map()) -> list().
